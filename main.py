...
@app.get("/sb/{table}")
async def sb_proxy(table: str, request: Request):
    """Supabase proxy — admin panel uchun CORS muammosini hal qiladi"""
    query = str(request.url.query)
    q = "?"+query if query else ""
    data = await sb_get(table, q)
    return data
...
async def webhook(req: Request):
    try:
        ...
        if text.startswith("/start"):
            ...
            if is_new and ref_param and ref_param != tg_id:
                ref_u = await sb_get("foydalanuvchilar", f"?tg_id=eq.{ref_param}&select=balans,rol,ism")
                if ref_u and ref_u[0].get("rol") == "usta":
                    new_bal = (ref_u[0].get("balans") or 0) + 5000
                    await sb_patch("foydalanuvchilar", f"?tg_id=eq.{ref_param}", {"balans": new_bal})
                    await sb_post("balans_tarixi", {"foydalanuvchi_id": ref_param, "tur": "referral", "miqdor": 5000, "izoh": f"Referral: {name}"})
                    await send(ref_param, f"🎁 <b>+5,000 so'm bonus!</b>\n{name} sizning havolangiz orqali kirdi!")
            ...
            if len(parts) == 3:
                ...
                try:
                    ...
                    if u:
                        ...
                        # Ustaga xabar
                        await send(target_tg_id,
                            f"✅ <b>To'lovingiz tasdiqlandi!</b>\n\n"
                            f"💰 Hisobingizga {summa:,} so'm qo'shildi.\n"
                            f"💳 Yangi balans: {new_bal:,} so'm"
                        )
                        await send(chat_id, f"✅ {u[0].get('ism','Usta')} ga {summa:,} so'm qo'shildi. Yangi balans: {new_bal:,} so'm")
                    else:
